...
BCU has two moving pieces: `openrouter-codex` manages the Codex configuration, terminal model picker and installation; `bcu_router.py` handles model-based HTTP routing. Keep the macOS Keychain as the only persistent API-key store. No credentials, bearer tokens, cookies, prompts, generated text, private model selections, local configuration or runtime databases belong in commits or test output.
